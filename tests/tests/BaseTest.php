...
class BaseTest extends PHPUnit_TestCase {
	...
	function BaseTest( $name = 'BaseTest'){
		...
		if (!defined('DB_NAME')) {
			define('DB_NAME', Dataface_Application::getInstance()->_conf['_database']['name']);
		}
		startTimer();
		$this->db = df_db() or die("Could not connect to db");
	...
	function setUp(){
		...
		xf_db_query("
			CREATE TABLE IF NOT EXISTS Profiles (
				...
				) Engine=InnoDB;", $this->db) or die("Error creating table 'Profiles': ".xf_db_error($this->db));
		...
		xf_db_query("	
			CREATE TABLE IF NOT EXISTS ProfileProperties (
				...
				Handicap INT(5)
				) Engine=InnoDB;", $this->db) or die("Error creating table 'ProfileProperties':". xf_db_error($this->db));
