...
class IOTest extends BaseTest {
	...
	function test_add_existing_related_record(){
		...
		$destTables = $relatedCourse->_relationship->getDestinationTables();
		$destTableNames = array();
		foreach ($destTables as $destTable) {
			$destTableNames[] = $destTable->tablename;
		}
		asort($destTableNames);
		$this->assertEquals(array('Courses', 'Student_Courses'), $destTableNames, "courses relationship destination tables is incorrect.");
		
		$drecords = $relatedCourse->toRecords();
		$this->assertEquals(2, count($drecords), 'courses relationship in n:m so should include 2 records.');
		...
		$this->assertTrue(!PEAR::isError($res), "Error occurred while adding existing course: $errorMessage");
	...
	function test_import_data(){
		...
		if (basename($importTablename) === $importTablename) {
		...
		if ($importTablename == basename($importTablename)) {
			// We used to store the temporary import data
			// in a table, but we changed it to use a 
			// temporary file
			// This branch is from when we used to keep it
			// in a table.. to test the contents.
			$res = xf_db_query("SELECT * FROM `$importTablename`", $this->db);
			if ( !$res ){
	...
	function test_loadRecordById(){
		$io = new Dataface_IO('Profiles');
		$record = $io->loadRecordById('Profiles?id=10');
		$this->assertEquals(
			'John',
			$record->val('fname')
			);
		...
		$record2 = $io->loadRecordById('Profiles/appointments?id=10&appointments::id=2');
		$this->assertEquals(
			'Teacher',
	...
	function test_removeRelatedRecord(){
		...
		$io = new Dataface_IO('Profiles');
		$record = $io->loadRecordById('Profiles/appointments?id=10&appointments::id=2');
		$res = $io->removeRelatedRecord($record);
		$this->assertTrue($res);
		$appt = df_get_record('Appointments', array('id'=>'=2'));
		$this->assertTrue($appt instanceof Dataface_Record, "Failed to load appointment.  It should still exist even though it was removed from the relationship.");
		$this->assertTrue(!$appt->val('profileid'), "Appointment should have a null profileid after it is removed from the relationship");
		$this->assertTrue(xf_db_num_rows(xf_db_query("SELECT * FROM `Appointments` where `id`=2"))==1);
		$appt->setValue('profileid', 10);
		$appt->save();
		$record = $io->loadRecordById('Profiles/appointments?id=10&appointments::id=2');
		$this->assertTrue(isset($record), 'Failed to load related record after explicitly setting the foreign key field.');
		
		$res = $io->removeRelatedRecord($record,true);
		$this->assertTrue($res);
		$this->assertTrue(xf_db_num_rows(xf_db_query("SELECT * FROM `Appointments` where `id`=2"))==0);
		$appt = df_get_record('Appointments', array('id'=>'=2'));
		$this->assertTrue(!$appt, 'Should not be able to load appointment after it has been deleted');
	}
